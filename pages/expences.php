...
<body class="flex">
    <aside class="h-screen flex flex-col absolute z-40 left-0 top-0 bottom-0 w-64 bg-black text-white transition-transform duration-300 ease-in-out md:relative md:tranblack-x-0 hidden lg:block">
            ...
            <nav class="flex-1 h-full px-2 py-4 space-y-2 overflow-y-auto">
                ...
                <a href="expences.php" class="flex items-center gap-3 px-4 py-3 bg-[#70E000] text-black-400 hover:text-white hover:bg-black-800 rounded-lg transition-colors">
    ...
    <div class="modal Modify-expences-form w-full h-screen bg-black/30 fixed top-0 left-0 flex justify-center items-center <?php echo isset($_GET['edit_id']) ? '' : 'hidden'; ?>" >
        <form action="expences/modify-expences.php" method="POST" class="relative w-full max-w-116 max-h-80 md:max-h-128 bg-white rounded-xl shadow-md px-4 py-8 flex flex-col items-center gap-4 overflow-y-auto">
            ...
            <input type="hidden" name="id" value="<?php echo $expence_data ? $expence_data['id'] : ''; ?>">
            
            <div class="flex flex-col w-full gap-1">
                <label for="">Montant:</label>
                <input type="text" name="montant" pattern="[0-9]{1,}" value="<?php echo $expence_data ? htmlspecialchars($expence_data['montant']) : ''; ?>" placeholder="Enter the amount of Revenu" class="p-2 bg-gray-200 rounded border border-gray-300" required>
            </div>
            <div class="flex flex-col w-full gap-1">
                <label for="">Date:</label>
                <input type="date" name="Date" value="<?php echo $expence_data ? htmlspecialchars($expence_data['date_']) : ''; ?>" class="p-2 bg-gray-200 rounded border border-gray-300" required>
            </div>
            <div class="flex flex-col w-full gap-1">
                <label for="">Description:</label>
                <textarea name="description" placeholder="Enter the Description of Revenu" class="min-h-30 p-2 bg-gray-200 rounded border border-gray-300" required><?php echo $expence_data ? htmlspecialchars($expence_data['description']) : ''; ?></textarea>
            </div>
            <input type="submit" value="Update Revenu" class="w-full bg-black text-white rounded-xl p-4">
        </form>
    </div>
